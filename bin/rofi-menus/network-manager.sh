...
manage_wifi() {
    ...
    local chosen_network=$(echo -e "$formatted_list" | wofi --dmenu -i -p "Wi-Fi SSID: ")
    local ssid_index=-1
        ...
        case $action in
            "󰸋  Connect")
                ...
                if [[ $(echo "$saved_connections" | grep -Fx "$chosen_id") ]]; then
                    ...
                    local wifi_password=$(wofi --dmenu -P -p "Password: " < /dev/null)
...
manage_ethernet() {
    ...
    local chosen_device=$(echo -e "$eth_list" | wofi --dmenu -i -p "Select Ethernet device: ")
